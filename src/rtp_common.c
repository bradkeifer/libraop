...
*/
struct rtp_session *
rtp_session_new(int pktbuf_size, int sync_each_nsamples)
{
  struct rtp_session *session;

  if (!(session = calloc(1, sizeof(struct rtp_session)))) {
    LOG_ERROR("Unable to calloc session. %s", strerror(errno));
    return (struct rtp_session *)NULL;
  }
  ...
  if (!(session->pktbuf = calloc(session->pktbuf_size, sizeof(struct rtp_packet)))) {
    LOG_ERROR("Unable to calloc session packet buffer. %s", strerror(errno));
    free(session);
    return (struct rtp_session *)NULL;
  }
...
rtp_packet_next(struct rtp_session *session, size_t payload_len, int samples, char payload_type, char marker_bit)
{
  ...
  if (!pkt->data || payload_len > pkt->payload_size) {
    ...
    if (!pkt->data) {
      if (!(pkt->data = malloc(pkt->data_size))) {
        LOG_ERROR("Unable to malloc packet data. %s", strerror(errno));
        return (struct rtp_packet *)NULL;
      }
    }
    else if (!(pkt->data = realloc(pkt->data, pkt->data_size))) {
      LOG_ERROR("Unable to realloc packet data. %s", strerror(errno));
      return (struct rtp_packet *)NULL;
    }
    pkt->header  = pkt->data;
...
rtp_packet_get(struct rtp_session *session, uint16_t seqnum)
{
  ...
  if (session->pktbuf_len == 0)
    {
      LOG_DEBUG("Seqnum %" PRIu16 " requested, but buffer is empty", seqnum);
      return NULL;
    }
  ...
  if (! ((first <= last) ^ (first <= seqnum) ^ (seqnum <= last)))
    {
      LOG_DEBUG("Seqnum %" PRIu16 " not in buffer (have seqnum %" PRIu16 " to %" PRIu16 ")", 
        seqnum, first, last);
      return NULL;
    }
...
rtp_sync_packet_next(struct rtp_session *session, struct rtcp_timestamp cur_stamp, char type)
{
  ...
  if (!session->sync_packet_next.data)
    {
      if (!(session->sync_packet_next.data = malloc(RTCP_SYNC_PACKET_LEN))) {
        LOG_ERROR("Unable to malloc next sync packet. %s", strerror(errno));
        return (struct rtp_packet *)NULL;
      }
...
/*  LOG_DEBUG("SYNC PACKET cur_ts:%ld.%ld, cur_pos:%u, rtptime:%u, type:0x%x, sync_counter:%d",
...
/*
  LOG_DEBUG("RTCP PACKET vers=%d, padding=%d, len=%" PRIu16 ", payload_len=%zu, ssrc=%" PRIu32, pkt->version, pkt->padding, pkt->len, pkt->payload_len, pkt->ssrc);
  if (pkt->packet_type == RTCP_PACKET_APP)
    LOG_DEBUG("RTCP APP PACKET subtype=%d, name=%s", pkt->app.subtype, pkt->app.name);
  else if (pkt->packet_type == RTCP_PACKET_XR)
    LOG_DEBUG("RTCP XR PACKET block_type=%d, block_len=%" PRIu16, pkt->xr.block_type, pkt->xr.block_len);
*/

  return 0;

 packet_malformed:
  LOG_DEBUG("Ignoring incoming packet, packet is non-RTCP, malformed or partial (size=%zu)", size);
  return -1;
}
