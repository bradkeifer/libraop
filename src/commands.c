...
command_cb_sync(struct commands_base *cmdbase, struct command *cmd)
{
  ...
  if (pthread_mutex_lock(&cmd->lck)) {
    LOG_ERROR("Unable to obtain lock for cmd mutex");
    return;
  }
  ...
  if (pthread_cond_signal(&cmd->cond)) {
    LOG_ERROR("Unable to signal calling thread that the command execution has finished");
  }
  if (pthread_mutex_unlock(&cmd->lck)) {
    LOG_ERROR("Unable to release lock for cmd mutex");
  }
...
command_cb(int fd, short what, void *arg)
{
  ...
  if (ret != sizeof(cmd)) {
    LOG_ERROR("Error reading command from command pipe: expected %zu bytes, read %d bytes", sizeof(cmd), ret);
    event_add(cmdbase->command_event, NULL);
    return;
  }
...
send_command(struct commands_base *cmdbase, struct command *cmd)
{
  ...
  if (!cmd->func) {
    LOG_ERROR("Programming error: send_command called with command->func NULL!");
    return -1;
  }

  ret = write(cmdbase->command_pipe[1], &cmd, sizeof(cmd));
  if (ret != sizeof(cmd)) {
    LOG_ERROR("Bad write to command pipe (write incomplete)");
    return -1;
  }
...
{
    struct commands_base *cmdbase = NULL;
    int ret = 0;

    if (!(cmdbase = calloc(1, sizeof(struct commands_base)))) {
      LOG_ERROR("Unable to calloc cmdbase. %s", strerror(errno));
      goto error;
    }
    ...
    #endif
    if (ret < 0) {
      LOG_ERROR("Could not create command pipe: %s", strerror(errno));
      goto error;
    }
    ...
    if (!cmdbase->command_event)
      {
        LOG_ERROR("Could not create cmd event");
        goto error;
      }
    ...
    if (ret != 0)
      {
        LOG_ERROR("Could not add cmd event");
        goto error;
      }
...
error:
    ...
    return (struct commands_base *) NULL;
}
...
commands_exec_end(struct commands_base *cmdbase, int retvalue)
{
    ...
    LOG_ERROR("Command has %d pending events", current_cmd->pending);
    ...
    if (!pthread_cond_signal(&current_cmd->cond)) {
        LOG_ERROR("pthread_cond_signal() error. %s", strerror(errno));
    }
    if (!(pthread_mutex_unlock(&current_cmd->lck))) {
        LOG_ERROR("Unable to release command mutex lock. %s", strerror(errno));
    }
}
...
commands_exec_sync(struct commands_base *cmdbase, command_function func, command_function func_bh, void *arg)
{
    ...
    if (pthread_mutex_init(&cmd.lck, NULL)) {
      LOG_ERROR("Unable to init command mutex. %s", strerror(errno));
      goto error;
    }
    if (pthread_cond_init(&cmd.cond, NULL)) {
      LOG_ERROR("Unable to initialise command mutex condition. %s", strerror(errno));
      goto error;
    }

    if (pthread_mutex_lock(&cmd.lck)) {
      LOG_ERROR("Unable to obtain lock on command mutex. %s", strerror(errno));
      goto error;
    }
    ...
    if (ret < 0) {
      ...
      if (pthread_cond_wait(&cmd.cond, &cmd.lck)) {
        LOG_ERROR("pthread_cond_wait() error. %s", strerror(errno));
        goto error;
      }
    }
    if (pthread_mutex_unlock(&cmd.lck)) {
        LOG_ERROR("pthread_mutex_unlock() error. %s", strerror(errno));
        goto error;
    }

    if (pthread_cond_destroy(&cmd.cond)) {
        LOG_ERROR("pthread_cond_destroy() error. %s", strerror(errno));
        goto error;
    }
    if (pthread_mutex_destroy(&cmd.lck)) {
        LOG_ERROR("pthread_mutex_destroy() error. %s", strerror(errno));
        goto error;
    }
...
commands_exec_async(struct commands_base *cmdbase, command_function func, void *arg)
{
    ...
    if (!(cmd = calloc(1, sizeof(struct command)))) {
        LOG_ERROR("Unable to calloc cmdbase. %s", strerror(errno));
        return -1;
    }
    cmd->func = func;
